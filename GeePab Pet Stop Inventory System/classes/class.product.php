...
public function new_product_type($tname){
	...
	public function new_product($pname,$desc,$price,$type){
		...
		public function list_product_search($keyword) {
			$q = $this->conn->prepare('SELECT * FROM `products` WHERE `name` LIKE ?');
	...
	public function update_product($name, $description, $type_id, $price, $product_ID) {
		$sql = "UPDATE products SET name=:name, description=:description, type_id=:type_id, price=:price WHERE product_ID=:product_ID";
		$q = $this->conn->prepare($sql);
		$q->execute(array(':name'=>$name, ':description'=>$description, ':type_id'=>$type_id, ':price'=>$price, ':product_ID'=>$product_ID));
		return true;
	}
